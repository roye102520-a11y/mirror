...
import { useEmotionalAcknowledge } from "@/context/EmotionalAmbientContext";
...
function QuizCompanionAnchor({ children }: { children: ReactNode }) {
  return (
    <div data-mirror-companion-anchor="" className="relative w-full overflow-visible">
      {children}
    </div>
  );
}
...
export function QuizPageClient() {
  ...
  const acknowledgeChoice = useEmotionalAcknowledge();
  ...
  if (!storageReady) {
    return (
      <div className="flex min-h-screen flex-col">
        <SiteHeader />
        <main className="mx-auto w-full max-w-xl flex-1 overflow-visible px-4 py-24 text-sm text-[var(--muted)] sm:px-6">
          <QuizCompanionAnchor>加载中……</QuizCompanionAnchor>
        </main>
        <LocalPrivacyBar />
      </div>
    );
  ...
  if (isMainComplete && !scanOpenComplete) {
    ...
    return (
      <div className="flex min-h-screen flex-col">
        <SiteHeader />
        <main className="mx-auto w-full max-w-xl flex-1 overflow-visible px-4 py-8 sm:px-6 sm:py-12">
          <QuizCompanionAnchor>
  ...
  if (internet.phase === "offered") {
    return (
      <div className="flex min-h-screen flex-col">
        <SiteHeader />
        <main className="mx-auto w-full max-w-xl flex-1 overflow-visible px-4 py-8 sm:px-6 sm:py-16">
          <QuizCompanionAnchor>
            <QuizProgressBar
              ...
              {iq.options.map((opt, i) => (
                <li key={i}>
                  <button
                    ...
                    onClick={() => {
                      acknowledgeChoice();
                      submitInternetAnswer(internet.index, i);
                    }}
