...
export function ContradictionExercisePanel({ obsession }: { obsession: string }) {
  ...
  if (!open) {
    return (
      <div className="rounded-lg border border-dashed border-[var(--line)] bg-white p-4 shadow-sm">
        ...
        <button
          ...
          className="mt-4 rounded-md border border-[var(--line)] bg-white px-4 py-2 text-xs text-[var(--ink)] shadow-sm hover:border-[var(--accent)]"
        >
          展开练习说明
        </button>
      </div>
  ...
  return (
    <div className="rounded-lg border border-[var(--line)] bg-white p-5 shadow-sm sm:p-6">
